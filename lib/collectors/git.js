 ...
 * artifact and returns a summary of key metrics for use by other tools and agents.
 *
 ...
 *
 * Loads the cached artifact if present; otherwise runs a full init.
 ...
 * @param {Object} [options={}] - Options passed to repoIntel.init if no cache exists
...
async function collect(basePath, options = {}) {
  let map = repoIntel.load(basePath);
  if (!map) {
    map = await repoIntel.init(basePath, options);
  }
