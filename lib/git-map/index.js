...
async function update(basePath) {
  ...
  if (!existing) {
    throw new Error('No git-map found. Run init first.');
  }

  await binary.ensureBinary();

  const lastCommit = existing.git && existing.git.commit;
  const args = ['git-map', 'update'];
...
function status(basePath) {
  ...
  return {
    ...
    status: {
      ...
      updated: map.updated,
      commit: map.git && map.git.commit,
      branch: map.git && map.git.branch,
      totalCommits: (map.stats && map.stats.totalCommits) || 0,
      totalFiles: (map.stats && map.stats.totalFiles) || 0
    }
  };
}
