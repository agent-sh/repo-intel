 ...
 *
 * @param {string} basePath - Repository root
 * @param {string[]|string} files - Changed file paths (array, or comma-separated string)
...
function diffRisk(basePath, files) {
  // Accept both an array and a pre-joined / single string — a caller passing
  // one path as a string previously crashed on files.join.
  const list = Array.isArray(files) ? files.join(',') : String(files);
  return runQuery(basePath, ['diff-risk', '--files', list]);
}
 ...
 * Files that tend to change together, grouped.
 *
 * @param {string} basePath - Repository root
 * @returns {Array<{id: number, size: number, label: string, files: string[]}>}
 */
...
/**
 * Files that bridge communities (high betweenness centrality) — the
 * cross-cutting glue whose change ripples widest.
 *
 * @param {string} basePath - Repository root
 * @param {Object} [options={}]
 * @param {number} [options.limit=10] - Maximum number of results
 * @returns {Array<{path: string, betweenness: number, communities: number[]}>}
 */
...
/**
 * Look up which co-change community a file belongs to.
 *
 * @param {string} basePath - Repository root
 * @param {string} file - File path (relative to repo root)
 * @returns {{file: string, communityId: number, label: string}|null}
 */
...
/**
 * Composite health metrics for one community by id (from `communities`).
 *
 * @param {string} basePath - Repository root
 * @param {number|string} id - Community id
 * @returns {Object} Health metrics for the community
 */
...
/**
 * Every place execution can start: binaries, main fns, npm scripts, etc.
 *
 * @param {string} basePath - Repository root
 * @param {Object} [options={}]
 * @param {string[]|string} [options.files] - Restrict to these repo-relative files
 * @returns {Array<{path: string, kind: string, name: string}>}
 */
function entryPoints(basePath, options = {}) {
  const args = ['entry-points'];
  if (options.files) {
    const list = Array.isArray(options.files) ? options.files.join(',') : String(options.files);
    args.push('--files', list);
  }
  return runQuery(basePath, args);
}

/**
 * Project metadata: languages, CI, license, README presence.
 *
 * @param {string} basePath - Repository root
 * @returns {{language: string, languages: Object, ci: string|null, license: string|null, hasReadme: boolean}}
 */
function projectInfo(basePath) {
  return runQuery(basePath, ['project-info']);
}
